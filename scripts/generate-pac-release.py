...
def load_changelog() -> dict:
    ...
    if not path.exists():
        return {"schema": "pac.changelog.v1", "current_version": version(), "entries": []}
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        return {"schema": "pac.changelog.v1", "current_version": version(), "entries": []}
    data.setdefault("schema", "pac.changelog.v1")
...
def release_entry(ver: str) -> dict:
    ...
    return {"version": ver, "title": f"PAC v{ver}", "changes": changes}


def ensure_changelog(ver: str) -> dict:
    data = load_changelog()
    entries = data.setdefault("entries", [])
    if not any(str(entry.get("version")) == ver for entry in entries if isinstance(entry, dict)):
        entries.append(release_entry(ver))
