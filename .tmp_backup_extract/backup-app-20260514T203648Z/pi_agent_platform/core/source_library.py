...
def _read_component_metadata(folder: Path, rel: Path) -> dict[str, Any] | None:
    for name in ('pac-component.json', 'PAC_COMPONENT.json', 'component.json'):
        ...
        if not isinstance(data, dict):
            continue
        meta = {k: v for k, v in data.items() if k in {'id', 'title', 'description', 'kind', 'maintainers', 'repository', 'homepage', 'license', 'tags', 'entrypoint', 'runtime', 'version', 'is_workspace_container'}}
