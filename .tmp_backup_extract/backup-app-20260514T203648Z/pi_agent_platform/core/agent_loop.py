...
{"type":"tool_call","tool":"code_search","input":{"query":"search term","path":"."}}
{"type":"tool_call","tool":"code_explain","input":{"file_path":"path/to/file.py","focus":"optional_function_name"}}
{"type":"tool_call","tool":"code_review","input":{"file_path":"path/to/file.py"}}
{"type":"tool_call","tool":"code_diff","input":{"diff_text":"<git diff output>"}}
...
# ── Local LLM client for code analysis ───────────────────────────────────────
def _call_code_model(prompt: str, config: AppConfig, session: Session, max_tokens: int = 2048) -> str:
    """Call local model via LM Studio for code analysis."""
    # Always use gemma-4e4b on 192.168.0.52 for code analysis (only reachable local provider)
    provider = "http://192.168.0.52:1234"
    ...
    try:
        ...
        payload = json.dumps({
            "model": model,
            "messages": [
                {"role": "system", "content": "You are a code analysis assistant. Be concise and specific. Focus on actionable insights."},
...
_APPROVAL_POLL_INTERVAL = 2.0
_APPROVAL_TIMEOUT_SECONDS = 600  # 10 minutes
...
async def _wait_for_approval(task: Task) -> dict[str, Any] | None:
    """Poll task.metadata for a pending_approval_response, waiting up to APPROVAL_TIMEOUT_SECONDS.
    Returns the response dict {"decision": "approved"|"denied", "reason": "..."} or None on timeout.
    """
    elapsed = 0.0
    while elapsed < _APPROVAL_TIMEOUT_SECONDS:
        await asyncio.sleep(_APPROVAL_POLL_INTERVAL)
        elapsed += _APPROVAL_POLL_INTERVAL
        # Reload task from store to pick up external changes
        updated = store.get_task(task.id)
        if updated:
            task = updated
        resp = task.metadata.get("pending_approval_response")
        if resp is not None:
            # Clear it so the same task can be used for future approvals
...
async def _run_shell(session: Session, task: Task, command: str, config: AppConfig) -> tuple[str, bool]:
    ...
    if decision == "ask" and session.permission_profile != "full-control":
        tool_payload = {"tool": "shell", "input": {"command": command}}
        store.add_event(Event(
            session_id=session.id, task_id=task.id,
            type="tool_approval_required",
            message=f"Agent wants to run shell command",
            data={"tool": "shell", "command": command, "reason": reason},
        ))
        # Store pending tool so approval can resume it
        task.metadata["pending_tool"] = tool_payload
        task.metadata["pending_approval_reason"] = reason
        store.add_task(task)
        resp = await _wait_for_approval(task)
        if resp is None:
            return "TIMEOUT: approval request timed out after 10 minutes", False
        if resp.get("decision") == "denied":
            return f"DENIED by user: {resp.get('reason') or 'approval denied'}", False
        # Approved — fall through to execute
        store.add_event(Event(session_id=session.id, task_id=task.id, type="tool_approved", message=f"Shell command approved", data={"command": command}))
...
async def execute_tool(session: Session, task: Task, tool: str, inp: dict[str, Any], config: AppConfig) -> tuple[str, bool]:
    ...
    # --- deny path: return DENIED immediately, caller lets agent continue ---
    def _deny(msg: str) -> tuple[str, bool]:
        return f"DENIED: {msg}", False

    if tool == "list_files":
        if perm.file_read == "deny":
            return _deny("file reads are denied")
    ...
    # --- ask path: emit event, wait for approval, resume or deny ---
    async def _ask_approval(tool_name: str, tool_input: dict[str, Any], reason: str) -> tuple[str, bool]:
        """Shared inline-approval flow for ask-permission tools."""
        tool_payload = {"tool": tool_name, "input": tool_input}
        store.add_event(Event(
            session_id=session.id, task_id=task.id,
            type="tool_approval_required",
            message=f"Agent wants to use: {tool_name}",
            data={"tool": tool_name, "input": tool_input, "reason": reason},
        ))
        task.metadata["pending_tool"] = tool_payload
        task.metadata["pending_approval_reason"] = reason
        store.add_task(task)
        ...
        if resp.get("decision") == "denied":
            return f"DENIED by user: {resp.get('reason') or 'approval denied'}", False
        # Approved — fall through to actual tool execution below
        store.add_event(Event(session_id=session.id, task_id=task.id, type="tool_approved", message=f"{tool_name} approved", data={"tool": tool_name}))
        return None, False  # signal: proceed

    if tool == "write_file":
        if perm.file_write == "deny":
            return _deny("file writes are denied")
        if perm.file_write == "ask" and session.permission_profile != "full-control":
            result = await _ask_approval("write_file", inp, f"write file: {inp.get('path')}")
            if result[0] is not None:
                return result  # denied or timed out
            # approved — fall through to execute
        path = str(inp.get("path") or "")
    ...
    if tool == "shell":
        if "shell" not in allowed:
            return _deny("shell tool is not enabled for this session")
        if session.permission_profile == "full-control":
            # full-control: run without asking
            pass
        else:
            decision, reason = command_policy(str(inp.get("command") or ""), session, config)
            if decision == "deny":
                return _deny(reason or "command denied")
            if decision == "ask":
                result = await _ask_approval("shell", inp, reason or f"shell: {inp.get('command')}")
                if result[0] is not None:
                    return result
    ...
    if tool == "web_fetch":
        if "web_fetch" not in allowed and "internet" not in allowed:
            return _deny("web_fetch/internet tool is not enabled for this session")
        if perm.network == "deny":
            return _deny("network access is denied")
        if perm.network == "ask" and session.permission_profile != "full-control":
            result = await _ask_approval("web_fetch", inp, f"fetch URL: {inp.get('url')}")
            if result[0] is not None:
                return result
            # approved — fall through
    ...
    if tool == "web_search":
        if "web_search" not in allowed and "internet" not in allowed:
            return _deny("web_search/internet tool is not enabled for this session")
        if perm.network == "deny":
            return _deny("network access is denied")
        if perm.network == "ask" and session.permission_profile != "full-control":
            result = await _ask_approval("web_search", inp, f"web search: {inp.get('query')}")
            if result[0] is not None:
                return result
            # approved — fall through
    ...
    if tool == "code_search":
        ...
        import subprocess
        query = str(inp.get("query") or "")
        path = str(inp.get("path") or session.workspace_path or ".")
        try:
            result = subprocess.run(
                ["grep", "-rn", "--color=never", query, path],
                capture_output=True, text=True, timeout=30,
                cwd=session.workspace_path
            )
            matches = result.stdout.strip() if result.stdout else "(no matches found)"
        except subprocess.TimeoutExpired:
            return "Search timed out (> 30s)", False
        except Exception as e:
            matches = f"Search error: {e}"
        analysis = _call_code_model(
            f"Query: find code matching: {query}\nResults:\n{matches[:3000]}\n\n"
            "Briefly analyze what was found and notable patterns.",
            config, session
        )
    ...
    if tool == "code_explain":
        ...
        target = _safe_path(session, file_path)
        if not target.is_file():
            return f"File not found: {file_path}", False
        if target.stat().st_size > 100_000:
            return f"File too large ({target.stat().st_size} bytes). Max: 100KB", False
        ...
        explanation = _call_code_model(
            f"Explain this code{f' (focus: {focus})' if focus else ''}.\n{excerpt}",
            config, session
        )
        return f"Explanation of {file_path}{f' (focus: {focus})' if focus else ''}:\n{explanation}", False
...
Be specific and cite problematic lines/sections.
Code ({target.name}):
{content[:5000]}""",
            config, session
        )
    ...
    if tool == "code_diff":
        diff_text = str(inp.get("diff_text") or "")
        if not diff_text:
            return "No diff text provided. Pass diff_text as input.", False
        explanation = _call_code_model(
            f"""Explain what this code diff changes. Be specific about:
- What was added, removed, or modified
- Why this change might have been made
