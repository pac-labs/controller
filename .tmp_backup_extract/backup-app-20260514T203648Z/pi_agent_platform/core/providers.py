...
def provider_public(config: AppConfig) -> dict[str, Any]:
    ...
    for name, p in config.providers.items():
        d = p.model_dump(exclude={"api_key"})
        live = _lmstudio_live_status(p)
        d["live_status"] = "online" if live["live"] else "offline"
        d["live_error"] = live["error"]
        d["loaded_models"] = live["loaded_models"]
        result[name] = d
    return result
...
def _lmstudio_live_status(provider: ProviderConfig) -> dict[str, Any]:
    """Probe LM Studio management API to get real server status and loaded models.
    
    Returns live=True with loaded model names if LM Studio is reachable and has models loaded,
    or live=False with error reason if unreachable.
    """
    if provider.type != 'lmstudio':
        return {"live": None, "loaded_models": [], "error": None}
    # Build base URL (strip /v1 suffix) and try v0 then v1
    base = (provider.base_url or 'http://localhost:1234').strip().rstrip('/')
    if base.endswith('/v1'):
        base = base[:-3].rstrip('/')
    for ver in ("v0", "v1"):
        url = f"{base}/api/{ver}/models"
        try:
            req = Request(url, headers={"Accept": "application/json"})
            with urlopen(req, timeout=5) as r:
                body = json.loads(r.read().decode() or "{}")
                # v0 uses {"models": [...]}, v1 uses {"data": [...]}
                ...
                for m in models:
                    ...
                    # v0: state in {"loaded"} or loaded_instances non-empty
                    # v1: state == "loaded"
                    state = m.get("state", "")
                    instances = m.get("loaded_instances", [])
                    if state == "loaded" or instances:
                        loaded.append(mid)
                return {"live": True, "loaded_models": loaded, "error": None}
        except Exception:
            continue
    return {"live": False, "loaded_models": [], "error": "unreachable"}
