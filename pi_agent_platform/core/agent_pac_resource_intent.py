...
def parse_pac_resource_plan(prompt: str) -> PacResourcePlan:
    ...
    lower = compact.lower()
    if not _looks_like_pac_resource_request(lower):
    ...
    needs_session = "session" in lower
...
def workspace_profile_action(plan: PacResourcePlan) -> dict[str, object]:
...
def session_action(plan: PacResourcePlan) -> dict[str, object]:
    payload: dict[str, object] = {
        "name": plan.name,
        "workspace_profile": plan.name,
        "permission_profile": "full-control",
        "expose_platform_tools": True,
        "metadata": {"purpose": "programming", "created_from_workspace_request": True},
    }
    return {"type": "tool_call", "tool": "pac_create_session", "input": payload}


def _looks_like_pac_resource_request(lower: str) -> bool:
    compact = " ".join(str(lower or "").split())
    if not compact.startswith(("create ", "add ", "make ", "set up ", "setup ", "please create ", "please add ")):
        return False
    ...
    return has_resource_term and has_pac_or_repo_context
